...
- 2026-05-03 — **Flashcards curadas por dataset**: cada `db-*.json` puede declarar un array `flashcards` (formalizado en `question-bank.schema.json`) con `{id, topicId, question, answer, origin}`. Solo se muestran flashcards curadas; NO se derivan flashcards desde preguntas de test (los enunciados de test no se adaptan al formato pregunta/respuesta). Las preguntas deben ser autocontenidas, específicas y memorizables; el schema rechaza formulaciones genéricas como “¿Qué idea clave debes recuperar del tema X?”. Datasets sin `flashcards` simplemente no tendrán tarjetas en la sección Flashcards. Archivos: `public/api/question-bank.schema.json`, `src/lib/storage.ts`, `public/api/db-uah-c1-tema-*.json`.
...
- **Flashcards**: solo se muestran flashcards curadas declaradas en los datasets. No se derivan desde preguntas de test. No se persisten en localStorage.
